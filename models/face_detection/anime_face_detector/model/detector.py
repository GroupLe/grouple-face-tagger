...
class LFFDDetector(object):
    def __init__(self, module_path, config, use_gpu=False):
        self.config = config
        path = Path(module_path)
        with open(path.joinpath(self.config["lffd_config_path"]), "r") as f:
            lffd_config = json.load(f)
        self.predictor = LFFDPredictor(
            mxnet=mx,
            symbol_file_path=path.joinpath(self.config["symbol_path"]),
            model_file_path=path.joinpath(self.config["model_path"]),
